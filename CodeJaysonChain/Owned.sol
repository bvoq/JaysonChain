...
function createAsset() public returns(uint256) {
	    Asset newAsset = Asset(msg.sender, new attribute[](0) , ??? );
...
contract JaysonChain is Owned {
	...
	function createAsset() public returns(uint256) {
	    ...
	    Asset memory newAsset = Asset({owner:msg.sender, assetIndex:allAssets.length, unixTime:now, internalTimeStamp:globalInternalTimeStamp, history: new Attribute[](0) });
	    allAssets.push(newAsset);
